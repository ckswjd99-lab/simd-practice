...
int main() {
  uint32_t n = 100*BLOCK_SIZE; // rows in A
  uint32_t m = 100*BLOCK_SIZE; // cols in B
  uint32_t k = 100*BLOCK_SIZE; // cols in a and rows in b
  float32_t A[n*k];
  float32_t B[k*m];
  float32_t C[n*m];
  float32_t D[n*m];
  ...
  printf("     SISD vs. SIMD(Neon)\n");
  printf("Task: Matrix Multiplication\n");
  printf("Matrix size: %dx%d, %dx%d\n", n, k, k, m);
  printf("===============================\n");
  start_c = clock();
  matrix_multiply_c(A, B, E, n, m, k);
  end_c = clock();
  float elapsed_c = (float)(end_c - start_c)/CLOCKS_PER_SEC;
  printf("C:\t %f\n", elapsed_c);
  clock_t start_neon, end_neon;
  start_neon = clock();
  matrix_multiply_neon(A, B, D, n, m, k);
  end_neon = clock();
  float elapsed_neon = (float)(end_neon - start_neon)/CLOCKS_PER_SEC;
  printf("Neon:\t %f\n", elapsed_neon);
  //print_matrix(D, n, m);
  c_eq_neon = matrix_comp(E, D, n, m);
  printf("===============================\n");
  printf("Neon equal to C? (epsilon=%f) %d\n", EPSILON, c_eq_neon);
  printf("Neon is %.2f%% times faster than plain C.\n", elapsed_c/elapsed_neon*100);
}